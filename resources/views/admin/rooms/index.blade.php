...
@section('content')
    <div class="container mx-auto px-4 py-6 bg-white w-full max-w-6xl">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.rooms.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg shadow-md hover:bg-blue-700">
                Create Room
            </a>
        </div>
            ...
            @foreach ($rooms as $room)
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="py-3 px-6 text-center">
                        <a href="{{ route('admin.rooms.edit', ['room' => $room->id]) }}" class="bg-yellow-500 text-white px-3 py-1 rounded shadow hover:bg-yellow-600">Edit</a>
                        <form action="{{ route('admin.rooms.destroy', ['room' => $room->id]) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
