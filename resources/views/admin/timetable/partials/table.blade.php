@foreach($rooms as $room)
    <tr class="border-b hover:bg-gray-50">
        ...
        <td class="p-4 text-center">
            <a href="{{  route('admin.timetable_edit', ['room' => $room->id])  }}" class="text-blue-500 hover:text-blue-700">
                <i class="fa-solid fa-pen-to-square"></i>
            </a>
        </td>
        <td class="p-4 text-center">
            <form action="{{route('admin.timetable_delete_post', ['room' => $room->id])  }}" method="POST" class="delete-form">
                @csrf
                @method('DELETE')
